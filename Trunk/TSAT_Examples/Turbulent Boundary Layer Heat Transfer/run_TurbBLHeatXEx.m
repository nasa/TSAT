...
%   TurbBLHeatXEx.slx model to run, executes the model, and displays the 
...
dt = 5; %Time-step [sec]
tsim = 400; %simulation time [sec]

% Boundary Layer Variables
Pr = 0.72; %Prantl Number
xBL = linspace(0.05,3,50); %locations along the surface to compute the boundary 
                        %layer [ft]
n = 30; %number of element to show in the boundary layer going out from the
        %surface
Te = 550; %temperature of the air at the edge of the boundary layer
rho_air = 0.00237; %denisty of air [slug/ft^3]
U = 50; %speed of the air flowing over the plate [ft/sec]
...
figure()
plot(u.Data(:,end,end)/U,Y.Data(:,end,end)/del.Data(:,end,end),'-b','LineWidth',2)
...
h1 = axes('Units','pixels','Position',axesPosition,...
          ...
          'XLim',xLimit,'YLim',[0 1500],'NextPlot','add');
h2 = axes('Units','pixels','Position',axesPosition+yWidth.*[-1 0 1 0],...
          'Color','none','XColor','k','YColor','m',...
          'XLim',xLimit+[xOffset 0],'YLim',[0 0.005],...
...
vidObj = VideoWriter('TurbBLheatXMovie.mp4','MPEG-4');
vidObj.FrameRate = 10;
open(vidObj);
j = 0;
for i = 1:length(TwmTe.Time)
